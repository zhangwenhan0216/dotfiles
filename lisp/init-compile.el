...
;; 使用
;; 1. M-x compile 输入编译命令（如 make, npm build）
;; F6 重新编译
...
(use-package alert
  ...
  :hook (compilation-finish-functions . sanityinc/alert-after-compilation-finish))
...
(use-package compile
  ...
  :bind ([f6] . recompile)
  :config
  ;; Save compilation buffer reference
  ...
  ;; Smart recompile in last compilation buffer
...
;; Shell command output in view-mode
